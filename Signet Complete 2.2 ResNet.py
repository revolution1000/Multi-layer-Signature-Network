...
"""
Following the design of 2.1. This version add residul connections to the network.
"""
...
class SignatureBlock(nn.Module):
    def __init__(self, d_in, m, S, K):
        ...
        # Normalization to stabilize the product-based output
        self.norm = nn.LayerNorm(S)
        
        # Learnable scale factor (initialized small to help training start linearly)
        self.gamma = nn.Parameter(torch.ones(1) * 0.1)
        
        # Residual shortcut: projects input to match signature dimension S
        self.shortcut = nn.Linear(d_in, S) if d_in != S else nn.Identity()

    def forward(self, x):
        # 1. The "Signature" Path
        z = self.linear(x)
        
        # Using absolute value or clamping can prevent NaNs if K has decimals
        # powered = torch.abs(z).unsqueeze(1) ** self.K.unsqueeze(0)
        powered = z.unsqueeze(1) ** self.K.unsqueeze(0)
        signature = powered.prod(dim=-1) 
        
        # 2. Apply Normalization and Scaling
        signature = self.norm(signature)
        
        # 3. Residual Connection: x_next = shortcut(x) + gamma * signature(x)
        return self.shortcut(x) + self.gamma * signature
...
class SignatureNetwork(nn.Module):
    def __init__(self, d_0, m, S, N, output_dim, K):
        ...
        self.blocks = nn.ModuleList([
            SignatureBlock(d_0 if i == 0 else S, m, S, K) 
            for i in range(N)
        ])
