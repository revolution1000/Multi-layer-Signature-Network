...
1. Using ReLu activation function to prevent negative value and thus the  multivalue of exponential.
2. The exponent matrix K can take real value. Including negative exponents and noninteger exponents.
3. Therefore to implement the model, we have better preprocess the data so that the ReLu won't destroy to much polymomial features.
"""
...
class SignatureBlock(nn.Module):
    def __init__(self, d_in, m, S):
        ...
        self.K = nn.Parameter(torch.randn(S, m) *0.02)
        self.epsilon = 1e-6
    ...
    def forward(self, x):
        ...
        o_log = torch.log(o + self.epsilon)
        log_signature = F.linear(o_log, self.K)
